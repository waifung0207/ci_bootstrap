...
function form_group_checkbox($name, $label, $checked = FALSE)
{
	...
	return "<div class='form-group'>".form_checkbox($name, $label, $checked)."</div>";
}
...
function form_group_link($label, $url)
{
	...
	return "<div class='form-group'><a href='$href'>$label</a></div>";
}

// Submit button (overrided)
function form_submit($label = 'Submit', $block = FALSE)
{
	$block = $block ? 'btn-block' : '';
	return "<button class='btn btn-primary $block' type='submit'>$label</button>";
}
